...
BEGIN
  MultiUse = -1  'True
END
Attribute VB_Name = "Inst_IRSwaption"
...
Public Sub Initialize(fld_ParamsInput As InstParams_SWT, Optional dic_CurveSet As Dictionary = Nothing, _
    ...
    If UCase(fld_Params.Exercise) = "BERMUDAN" Then
        'Replace Value Date for BSwap only
        lng_ValDate = fld_Params.ValueDate
...
Dim arr_SmartMult As Variant
ReDim arr_SmartMult(1 To UBound(arr_TimeStep)) As Variant
...
arr_output = PDE(enu_Direction, 1, arr_SpotStep, arr_TimeStep, arr_TimeLabel, dbl_MeanRev, arr_sigma, arr_payoff_t_a, arr_UndVal_A, arr_payoff_t_b, arr_UndVal_B, arr_SmartMult, bln_Smooth)
...
Dim arr_payoff() As Variant
Dim arr_Payoff_Time() As Variant
...
For int_cnt = col_PeriodStart.count To 1 Step -1
    ...
    Else
        int_i = int_i + 1
        ReDim Preserve arr_payoff(1 To UBound(arr_SpotStep), 1 To int_i) As Variant
        ReDim Preserve arr_Payoff_Time(1 To int_i) As Variant
