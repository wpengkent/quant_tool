...
' ## MEMBER DATA
' Components
Private irl_BondLeg As IRLeg_NID, scf_Purchase As SCF
...
Public Sub Initialize(fld_ParamsInput As InstParams_BND, Optional dic_CurveSet As Dictionary = Nothing, _
    ...
    Dim fld_BondLegParams As BondParams
    ...
    'Dim ygd_Generators As BondParams: ygd_Generators = ygs_Generators.Lookup_YGenerator(fld_ParamsInput.SW_YieldGenerator)
    'Dim fld_BondLegParams As IRLegParams: fld_BondLegParams = ygd_Generators
    fld_BondLegParams = ygs_Generators.Lookup_YGenerator(fld_ParamsInput.YieldGenerator)
    '''''

    ' Stored components
    With fld_BondLegParams
        .ValueDate = lng_SpotDate
        .StubType = fld_ParamsInput.StubType
        .IsLongCpn = fld_ParamsInput.IsLongCpn
        .TradeDate = fld_ParamsInput.TradeDate
    ...
    Set irl_BondLeg = New IRLeg_NID
    Call irl_BondLeg.Initialize(fld_BondLegParams, dic_CurveSet, dic_GlobalStaticInfo)
    ...
    Set dic_CurveDependencies = Convert_MergeDicts(dic_CurveDependencies, irl_BondLeg.CurveDependencies)
End Sub
...
Public Property Get marketvalue() As Double
    ...
    marketvalue = irl_BondLeg.marketvalue * SpotDF * FXConvFactor() * int_Sign

    If scf_Purchase.PmtDate > lng_SpotDate Then
        marketvalue = marketvalue - scf_Purchase.CalcValue(lng_ValDate, lng_SpotDate, str_CCY_PnL) * int_Sign
    End If
...
Public Property Get Cash() As Double
    ...
    'Cash = (irl_BondLeg.CustomCashValuation(lng_ValDate, irc_SpotDisc) * FXConvFactor() - scf_Purchase.CalcValue(lng_ValDate, lng_SpotDate, str_Ccy_PnL)) * int_Sign

    Cash = irl_BondLeg.CustomCashValuation(lng_ValDate, irc_SpotDisc) * FXConvFactor() * int_Sign

    If scf_Purchase.PmtDate <= lng_SpotDate Then
        Cash = Cash - scf_Purchase.CalcValue(lng_ValDate, lng_SpotDate, str_CCY_PnL) * int_Sign
    End If
...
Public Property Get Yield() As Double
    Yield = irl_BondLeg.BondYield
End Property

Public Property Get Duration() As Double
    Duration = irl_BondLeg.BondDuration
End Property

Public Property Get ModifiedDuration() As Double
    ModifiedDuration = irl_BondLeg.BondModifiedDuration
End Property
...
Public Function Calc_DV01(str_curve As String, Optional int_PillarIndex As Integer = 0) As Double
    ...
    If dic_CurveDependencies.Exists(str_curve) Then
        ...
        If int_PillarIndex = 0 Then
            ...
            dbl_Output = irl_BondLeg.Calc_DV01_Analytical(str_curve) * SpotDF * FXConvFactor() * int_Sign
        Else
            ' Use finite differencing instead of analytical
            ' Store shifted values
            Dim dbl_Val_Up As Double, dbl_Val_Down As Double
            Call irl_BondLeg.SetCurveState(str_curve, CurveState_IRC.Zero_Up1BP, int_PillarIndex)
            Call scf_Purchase.SetCurveState(str_curve, CurveState_IRC.Zero_Up1BP, int_PillarIndex)
            dbl_Val_Up = Me.marketvalue

            Call irl_BondLeg.SetCurveState(str_curve, CurveState_IRC.Zero_Down1BP, int_PillarIndex)
            ...
            Call irl_BondLeg.SetCurveState(str_curve, CurveState_IRC.Final)
...
Private Property Get FXConvFactor() As Double
    ...
    FXConvFactor = fxs_Spots.Lookup_DiscSpot(irl_BondLeg.Params.CCY, str_CCY_PnL)
End Property
...
Public Sub HandleUpdate_IRC(str_CurveName As String)
    ...
    Call irl_BondLeg.HandleUpdate_IRC(str_CurveName)
End Sub
...
Public Sub SetValDate(lng_Input As Long)
    ...
    Call FillSpotDate
    Call irl_BondLeg.SetValDate(lng_SpotDate)
End Sub
...
Public Sub OutputReport(wks_output As Worksheet)
    With wks_output
        ...
        Call irl_BondLeg.OutputReport_Bond(.Range("A1"), lng_ValDate, str_CCY_PnL, int_Sign, irc_SpotDisc.CurveName, False, scf_Purchase)
