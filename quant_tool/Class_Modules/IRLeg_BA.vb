...
Private Sub FillFlowDurations()
    ...
    For int_ctr = 1 To lngLst_PmtDates.count
        ...
        dbl_ActiveDuration = 0 ' Calc_yearfrac(fld_Params.ValueDate, lngLst_PmtDates(int_Ctr), str_Daycount_Duration, fld_Params.PmtFreq, bln_ActActFwdGeneration) 'QJK code mod 01102014
...
Dim dic_SecantOutputs As Dictionary: Set dic_SecantOutputs = New Dictionary
CalcYield = Solve_Secant(ThisWorkbook, "SolverFunc_YieldBA", dic_SecantParams, fld_Params.Yield, 0.01, dbl_Target, 0.00000000001 * fld_Params.Notional, 100, -100, dic_SecantOutputs)
...
ElseIf fld_Params.YieldCalc = "AIBD" Or fld_Params.YieldCalc = "AIBD (Effective flows)" Or fld_Params.YieldCalc = "AIBD AM/Step" Or fld_Params.DaycountConv = "ACT/ACT" Then
...
Public Function OutputReport_Bond(rng_OutputTopLeft As Range, lng_ValDate As Long, str_CCY_PnL As String, _
    ...
    With rng_OutputTopLeft
       ...
       ' Else
        ...
        'End If
    End With
    'QJK Comment 01102014 only outputs from a1:b14
    ...
    With rng_OutputTopLeft
        ...
        ' Rates
        'If fld_Params.PExch_Start = True Then int_ActiveRow = int_ActiveRow + 1 'QJK comment 01102014
        int_ActiveCol = int_ActiveCol - 2
        For int_ctr = 1 To 1 ' lngLst_PeriodEnd.count
            .Offset(int_ctr + int_ActiveRow, int_ActiveCol).Value = lngLst_PeriodStart(int_ctr)

            int_ActiveCol = int_ActiveCol + 1
           ' .Offset(int_Ctr + int_ActiveRow, int_ActiveCol).Value = lngLst_PeriodEnd(int_Ctr) 'QJK comment 01102014
            .Offset(int_ctr + int_ActiveRow, int_ActiveCol).Value = lngLst_PeriodEnd(lngLst_PeriodEnd.count)
            'If bln_IsFixed = True Then
            int_ActiveCol = int_ActiveCol + 1
                'int_ActiveCol = int_ActiveCol + 3
                .Offset(int_ctr + int_ActiveRow, int_ActiveCol).Value = dblLst_Rates(int_ctr)
           ...
           ' End If
            ...
            .Offset(int_ctr + int_ActiveRow, int_ActiveCol).Resize(1, 2).NumberFormat = str_DateFormat
        Next int_ctr

      '  If fld_Params.PExch_Start = True Then int_ActiveRow = int_ActiveRow - 1   'Qcomment 01102014
    End With
    ...
    ' Optional principal exchange at start   'Qcomment 01102014
    'If fld_Params.PExch_Start = True Then
      '  int_ActiveRow = int_ActiveRow + 1
      '  With rng_OutputTopLeft
        '    .Offset(int_ActiveRow, int_ActiveCol).Value = fld_Params.SwapStart
    ...
    With rng_OutputTopLeft
       ...
       ' If fld_Params.PExch_Start = True Then
        ...
        For int_ctr = 1 To 1 ' int_RangeHeight QJK mod 01102014
            int_ActiveRow = int_ActiveRow + 1

            If int_ctr Mod int_CalcsPerPmt = 0 Then
                int_ActiveIndex = int_ActiveIndex + 1
                .Offset(int_ActiveRow, int_ActiveCol).Value = lngLst_PmtDates(int_RangeHeight) 'lngLst_PmtDates(int_ActiveIndex)
                .Offset(int_ActiveRow, int_ActiveCol).NumberFormat = str_DateFormat
                Call dic_TempAddresses.Add("PmtDate", .Offset(int_ActiveRow, int_ActiveCol).Address(False, False))

                int_ActiveCol = int_ActiveCol + 1
                If intLst_IsMV(int_RangeHeight) = 1 Then
                ...
                int_ActiveCol = int_ActiveCol + 1
                .Offset(int_ActiveRow, int_ActiveCol).Value = dblLst_IntFlows(int_RangeHeight) 'dblLst_IntFlows(int_ActiveIndex) QJK comment 01102014
                .Offset(int_ActiveRow, int_ActiveCol).NumberFormat = str_CurrencyFormat

                int_ActiveCol = int_ActiveCol + 1
                .Offset(int_ActiveRow, int_ActiveCol).Value = dblLst_PrnFlows(int_RangeHeight)   'dblLst_PrnFlows(int_ActiveIndex) QJK comment 01102014
                .Offset(int_ActiveRow, int_ActiveCol).NumberFormat = str_CurrencyFormat
   ...
   ' Else
        rng_MV.Formula = "=" & str_Sign & "SUMPRODUCT((" & rng_FlowTypes(1, 1).Address(False, False) & "=""MV"")*(" _
            & rng_FlowTypes(1, 1).Offset(0, 1).Address(False, False) & "+" & rng_FlowTypes(1, 1).Offset(0, 2).Address(False, False) _
            & ")*(" & rng_FlowTypes(1, 1).Offset(0, 3).Address(False, False) & ")*" & rng_FlowTypes(1, 1).Offset(0, 4).Address(False, False) _
    ...
    'If bln_IsFuturesUnd = False Then
        rng_Cash.Formula = "=" & str_Sign & "SUMPRODUCT((" & rng_FlowTypes(1, 1).Address(False, False) & "=""CASH"")*(" _
            & rng_FlowTypes(1, 1).Offset(0, 1).Address(False, False) & "+" & rng_FlowTypes(1, 1).Offset(0, 2).Address(False, False) _
            & ")*(" & rng_FlowTypes(1, 1).Offset(0, 3).Address(False, False) & ")*(" & rng_FlowTypes(1, 1).Offset(0, 4).Address(False, False) _
