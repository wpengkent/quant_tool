...
Private dic_Cache_Spots As Dictionary
...
Public Sub Initialize(wks_Input As Worksheet, bln_DataExists As Boolean, Optional dic_StaticInfoInput As Dictionary = Nothing)
    ...
    Set dic_Cache_Spots = New Dictionary
...
Public Sub LoadRates()
    ...
    Debug.Assert map_Rules.Dict_SourceTables.Exists("EQSPT")
    Dim str_TableName As String: str_TableName = map_Rules.Dict_SourceTables("EQSPT")
...
Public Sub ClearCurve()
    Call Action_ClearBelow(rng_QueryTopLeft, 10)
End Sub

Public Sub ResetCache_Lookups()
    Call dic_Cache_Spots.RemoveAll
End Sub


' ## METHODS - LOOKUP
Public Function Lookup_Spot(str_Code As String, Optional bln_GetOrig As Boolean = False) As Double
    ...
    If dic_Cache_Spots.Exists(str_CacheKey) = True Then
        dbl_Spot = dic_Cache_Spots(str_CacheKey)
    Else
        ...
        Call dic_Cache_Spots.Add(str_CacheKey, dbl_Spot)
    End If

    Lookup_Spot = dbl_Spot
End Function
...
Public Sub Scen_ApplyBase()
    ...
    'Handle error for blank worksheet
    If rng_QueryTopLeft.Value = "" Then Exit Sub
